...
1. `backend/src/modules/patients/patients.service.ts` — delete the `[form-sync delta]` block from `update()`; after the update call `paymentsService.resyncPatient(id)` and recompute the entry date. `create()` keeps writing one Payment row per non-zero bucket and additionally accepts an optional `payments[]` array (each with its own required date).
