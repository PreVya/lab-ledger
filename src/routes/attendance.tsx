              ...
              <tr key={r.employee.id} className="border-t">
                ...
                <td className="p-2">
                  <Select value={r.current?.status ?? ""} onValueChange={(v) => setDraft(d => ({ ...d, [r.employee.id]: { status: v as AttendanceStatus, notes: d[r.employee.id]?.notes } }))}>
                    <SelectTrigger className="w-40"><SelectValue placeholder="unmarked" /></SelectTrigger>
                    <SelectContent>
...
function EmployeeFormDialog({ open, onOpenChange, employee }: { open: boolean; onOpenChange: (o: boolean) => void; employee: Employee | null }) {
  ...
  const initialized = useMemo(() => ({ id: employee?.id }), [employee?.id]);

  if (open && employee && initialized.id !== employee.id) {
    // no-op — form fields reset on open below via effect substitute
  }
  ...
  useMemo(() => {
    ...
    if (employee) {
      ...
      setName(""); setMobile(""); setDesignation(""); setMonthlySalary(""); setFile(null);
    }
  }, [open, employee]);
  ...
  async function save() {
    ...
    fd.set("name", name); fd.set("mobile", mobile); fd.set("designation", designation); fd.set("monthlySalary", monthlySalary || "0");
